...
from AlexaMusic.plugins.play.filters import command, app
def remove_if_exists(path):
    if os.path.exists(path):
        os.remove(path)


@Client.on_message(command(["/song", "نزل", "اغاني", "موسيقى", "تنزيل", "/music"]) & ~filters.edited)
...
async def video_downloader(_, message):
    ...
    try:
        ...
        traceback.print_exc()
        return await msg.edit(f"🥹 شنو رد تنزل \n√")
